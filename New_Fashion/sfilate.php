...
<link href="css/sfilateCSS.css" rel="stylesheet" type="text/css">
<script>
...
<body onload="cambia_colori('<?php echo $colore_sfondo; ?>','<?php echo $colore_elementi; ?>')">
  ...
  <div id="container">
  	<p><strong>Sfilate in programma</strong><br><br>
	<?php
	$linea=null;
	foreach($db->query("(SELECT * FROM sfilate) ORDER BY categoria") as $row) {
		if($row['categoria']!=$linea) {
			$linea=$row['categoria'];
			echo '<br/><u>Linea '.$row['categoria'].'</u><br>';
		}
		echo $row['data'].' alle '.$row['ora'].' presso '.$row['luogo'].'<br>';
	}
	?>
  	</p>
  </div>
