...
<html>
    <head>
        ...
        <title>New Fashion - modifica dettaglio</title>
        <style type = "text/css">
        html{
            padding: 10px;
            background: #ddd;
    ...
    <body>
        ...
        <div id="container">
            <div id="azioni">
                ...
                String taglia_n = request.getParameter("taglia_nuova");
                String quantita = request.getParameter("quantita");
                
                if((quantita != null) && (codice != "") && (taglia_n != "") && (quantita != "")) {
                    int c = Integer.parseInt(codice);
                    int q = Integer.parseInt(quantita);
                    boolean flag = dbConnection.aggiorna_dettaglio(c,taglia,q,taglia_n);
                    if(flag)
            ...
            <form method="post" action=""> 
                <h3><strong>Modifica dettaglio</strong></h3><br>
                <h3>Codice prodotto:  <% out.println(codice); %></h3>
                <%
                out.println("<input name=\"codice\" type=\"hidden\" value=\""+codice+"\"/>");
                out.println("<input name=\"taglia\" type=\"hidden\" value=\""+taglia+"\"/>");
                
                ResultSet dettaglio = dbConnection.getDettaglio(codice,taglia);
                for (dettaglio.beforeFirst(); dettaglio.next();) {
                    out.println("<h3>Taglia:</h3>");
                    out.println("<input name=\"taglia_nuova\" type=\"text\" size=\"40\" maxlength=\"50\" value=\""+dettaglio.getString("taglia")+"\" />");
                    out.println("<h3>Quantità:</h3>");
                    out.println("<input name=\"quantita\" type=\"text\" size=\"40\" maxlength=\"50\" value=\""+dettaglio.getString("quantita")+"\" />");
                }
                %>
