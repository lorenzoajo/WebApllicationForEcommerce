...
<html>
    <head>
        ...
        <title>New Fashion - preferenze utente</title>
        <style type = "text/css">
        html{
            padding: 10px;
            background: #ddd;
    ...
    <body>
        ...
        <div id="container">
            ...
            <table>
                <tr>
                  <th>Email utente</th>
                  <th>Linea preferita</th>
                  <th>Capo preferito</th> 
                  <th>Colore sfondo</th> 
                  <th>Colore elementi</th>
                </tr>
                <%
                new_fashion dbConnection = new new_fashion();
                ResultSet utente = dbConnection.getUtenti();
                for (utente.beforeFirst(); utente.next();) {
                    out.println("<td>"+utente.getString("email")+"</td><td>"+utente.getString("linea_preferita")+"</td><td>"
                        +utente.getString("capo_preferito")+"</td><td>"+utente.getString("colore_sfondo")+"</td><td>"
                        +utente.getString("colore_elementi")+"</td></tr>");
                }
                %>
            </table>
        </div>
